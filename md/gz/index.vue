<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="改制产品状态" label-width="100px" prop="restructuringStatus">
        <el-select v-model="queryParams.restructuringStatus" placeholder="请选择改制产品状态" clearable>
          <el-option
            v-for="dict in dict.type.sys_cp_cpzt"
            :key="dict.value"
            :label="dict.label"
      ...
      <el-form-item label="产品物料编码" label-width="98px" prop="materialCode">
        <el-input
      ...
      <el-form-item label="产品物料名称" label-width="100px" prop="materialName">
        <el-input
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择状态" clearable>
          <el-option
            v-for="dict in dict.type.sys_cys_gzzt"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
...
<!--      <el-table-column label="操作" width="100" align="center" class-name="small-padding fixed-width" fixed="left">-->
...
<!--      </el-table-column>-->
      ...
      <el-table-column label="状态" width="80" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_cys_gzzt" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column label="牌号" align="center" width="90" prop="brand">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_rz_ph" :value="scope.row.brand"/>
        </template>
      </el-table-column>
      <el-table-column label="厚度（mm）" width="110" align="center" prop="blankThickness" />
      <el-table-column label="宽度（mm）" width="110" align="center" prop="blankWidth" />
      <el-table-column label="重量" width="90" align="center" prop="weight" />
      <el-table-column label="改制前产品厚度" width="150" align="center" prop="productThickness" />
      <el-table-column label="改制前产品宽度" width="150" align="center" prop="productWidth" />
      <el-table-column label="改制前产品状态" width="150" align="center" prop="productStatus">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_cp_cpzt" :value="scope.row.productStatus"/>
        </template>
      </el-table-column>
      <el-table-column label="改制后产品厚度" width="150" align="center" prop="restructuringThickness" />
      <el-table-column label="改制后产品宽度" width="150" align="center" prop="restructuringWidth" />
      <el-table-column label="改制后产品状态" width="150" align="center" prop="restructuringStatus">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_cp_cpzt" :value="scope.row.productStatus"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="公差类型" align="center" prop="toleranceType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_cys_gc" :value="scope.row.toleranceType"/>
        </template>
      </el-table-column>
      <el-table-column label="公差要求" align="center" prop="toleranceCount" />
...
<!--            <WorkorderSelect :filterCondition="filterCondition" ref="woSelect" @onSelected="onWorkorderSelected"></WorkorderSelect>-->
          ...
          <el-col :span="8">
            <el-form-item label="厚度" prop="blankThickness">
              <el-input-number :min="0.00" v-model="form.blankThickness" placeholder="请输入厚度" />
              <el-span>&nbsp;mm</el-span>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="宽度" prop="blankWidth">
              <el-input-number :min="0.00" v-model="form.blankWidth" placeholder="请输入宽度" />
              <el-span>&nbsp;mm</el-span>
            </el-form-item>
          </el-col>
          <el-col :span="8">
          ...
          <el-col :span="8">
            <el-form-item label="改制后产品厚度" label-width="120px" prop="restructuringThickness">
              <el-input-number :min="0.00" v-model="form.restructuringThickness" placeholder="请输入产品厚度" />
              <el-span>&nbsp;mm</el-span>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="改制后产品宽度" label-width="120px" prop="restructuringWidth">
              <el-input-number :min="0.00" v-model="form.restructuringWidth" placeholder="请输入产品宽度" />
              <el-span>&nbsp;mm</el-span>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="改制后产品状态" label-width="120px">
              <el-select v-model="form.restructuringStatus" placeholder="请选择产品状态" >
                <el-option :label="''" :value="null"></el-option>
                <el-option
                  v-for="dict in dict.type.sys_cp_cpzt"
                  :key="dict.value"
...
<!--        <el-button type="primary" @click="" >改制提交</el-button>-->
        <el-button type="primary" @click="cysfun" v-if="!form.showSave">改制提交</el-button>
...
<script>
import { listGz, getGz, delGz, addGz, updateGz } from "@/api/mes/md/gz";
...
export default {
  ...
  dicts: ['sys_cys_gzzt', 'sys_cys_gc', 'sys_cp_cpzt', 'sys_rz_ph', 'sys_cys_gx'],
  data() {
    return {
      // 筛选条件，假设是根据工单状态进行筛选
      filterCondition: {
        gdType: '12',  // 比如筛选状态为 '12-改制' 的工单
      },
      // 遮罩层
      loading: true,
      // 选中数组
      ...
      queryParams: {
        pageNum: 1,
        pageSize: 10,
        gzCode: null,        workorderId: null,  tensileStrengthStart: null,  tensileStrengthEnd: null,   hardnessStart: null,     hardnessEnd: null,        workorderCode: null,        brand: null,        batchNumber: null,        weight: null,        blankThickness: null,        blankWidth: null,        productThickness: null,        productWidth: null,        productStatus: null,        restructuringThickness: null,        restructuringWidth: null,        restructuringStatus: null,        materialCode: null,        materialName: null,        toleranceType: null,        toleranceCount: null,        tensileStrength: null,        hardness: null,        status: null,        userId: null,        userName: null,      },
      // 表单参数
      form: {},
      // 表单校验
      rules: {
        ...
        ],        restructuringThickness: [
          { required: true, message: "改制产品厚度不能为空", trigger: "blur" }
        ],        restructuringWidth: [
          { required: true, message: "改制产品宽度不能为空", trigger: "blur" }
        ],        toleranceCount: [
          { required: true, message: "公差要求不能为空", trigger: "blur" }
        ],        tensileStrength: [
  ...
  computed: {
    modifiedRzList() {
      return this.gzList.map(item => {
        console.log("item.status = " + item.status);
        return {
          ...item,
          showButton: item.status !== '0'  // 根据 status 来设置 showButton 字段
        };
      });
    }
  },
  methods: {
    ...
    async cysfun() {
      ...
      try {
        ...
        await listScgdmx(queryGd).then(async response => {
          const scgdmx = response.rows[response.rows.length - 1];
          console.log("scgdmx = ", scgdmx);
          if (response.rows.length === 0) {
            ...
            const scgdcys = {
              workId: this.form.workorderId,
              workStatus: '1',
              thisProcess: scgdmx.nextProcess
            };
            console.log("scgdmx = ", scgdmx);
            const gxjd = this.dict.type.sys_cys_gx.find(option => option.value === scgdmx.nextProcess) || { label: 'defaultLH' };
            const response1 = await updateScgd(scgdcys);  // 使用局部变量 scgdcys
            this.$modal.msgSuccess("已进入" + gxjd.label + " 工序");
          }
        });
    ...
    pdScgd(event){
      ...
      listScgd(queryRz).then(response => {
        ...
        if(response.rows.length === 0){
          ...
          if(scgd.thisProcess !== '12' && scgd.workStatus !== '3'){
    ...
    getList() {
      this.loading = true;
      listGz(this.queryParams).then(response => {
        this.gzList = response.rows;
    ...
    reset() {
      this.form = {
        ...
        productWidth: null,
        productStatus: "0",
        restructuringThickness: null,
        restructuringWidth: null,
        restructuringStatus: "0",
        materialCode: null,
        materialName: null,
        toleranceType: null,
        toleranceCount: null,
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.gzId)
    ...
    handleUpdate(row) {
      this.reset(); // 设置默认初始值
      const gzId = row.gzId || this.ids;

      getGz(gzId).then(response => {
        const data = response.data || {};

        // 如果 userId 或 userName 是空的，就使用默认值
        const userId = data.userId || this.$store.state.user.userId;
        const userName = data.userName || this.$store.state.user.name;

        // 合并数据，保留原有默认值 + 接口返回数据
        this.form = {
          ...this.form,  // 保留 reset 中设置的其他默认字段
          ...data,       // 使用后端返回的字段
          userId,        // 覆盖为条件判断后的值
          userName
        };
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.gzId != null) {
            updateGz(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGz(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const gzIds = row.gzId || this.ids;
      this.$modal.confirm('是否确认删除改制编号为"' + gzIds + '"的数据项？').then(function() {
        return delGz(gzIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('md/gz/export', {
        ...this.queryParams
      }, `gz_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
