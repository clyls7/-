...
<!--      <el-form-item label="工单编码" prop="dsmCode">-->
<!--        <el-input-->
<!--          v-model="queryParams.dsmCode"-->
<!--          placeholder="请输入工单编码"-->
<!--          clearable-->
...
<!--      </el-form-item>-->
      ...
      <el-form-item label="铣面次数" prop="millingCount">
        <el-input
          v-model="queryParams.millingCount"
          placeholder="请输入铣面次数"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="铣前规格" prop="spBeforeMilling">
        <el-input
      ...
      <el-form-item label="铣后规格" prop="spAfterMilling">
        <el-input
      ...
      <el-form-item label="铣面时间" prop="millingTime">
        <el-date-picker clearable
          v-model="queryParams.millingTime"
          type="date"
...
<!--      <el-table-column label="工单编码" width="150" align="center" prop="dsmCode" />-->
      ...
      <el-table-column label="铣面次数" align="center" prop="millingCount" />
      <el-table-column label="铣前规格" align="center" prop="spBeforeMilling" />
      <el-table-column label="铣后规格" align="center" prop="spAfterMilling" />
      <el-table-column label="铣前重量" align="center" prop="spBeforeWeight" />
      <el-table-column label="铣后重量" align="center" prop="spAfterWeight" />
      <el-table-column label="铣面时间" align="center" prop="millingTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.millingTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="员工姓名" align="center" prop="userName" />
      <el-table-column label="质量判定" align="center" prop="qualityJudgment">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_smx_zlpd" :value="scope.row.qualityJudgment"/>
        </template>
      </el-table-column>
...
<!--          <el-col :span="8">-->
<!--        <el-form-item label="工单编码" prop="dsmCode">-->
<!--          <el-input v-model="form.dsmCode" placeholder="请输入双面铣工序编码" />-->
<!--        </el-form-item>-->
<!--          </el-col>-->
<!--          <el-col :span="8">-->
...
<!--                <el-button slot="append" @click="handleWorkorderSelect" icon="el-icon-search"></el-button>-->
        ...
        <el-form-item label="批次号" prop="batchNumber">
          ...
          <el-col :span="8">
        <el-form-item label="铣面次数" prop="millingCount">
          <el-input-number :min="1" v-model="form.millingCount" placeholder="请输入铣面次数" disabled/>
        </el-form-item>
          </el-col>
          <el-col :span="8">
        <el-form-item label="铣前规格" prop="spBeforeMilling">
          <el-input-number :min="0.00" v-model="form.spBeforeMilling" placeholder="请输入铣前规格" />
          <el-span>&nbsp;mm</el-span>
        </el-form-item>
          </el-col>
          <el-col :span="8">
        <el-form-item label="铣后规格" prop="spAfterMilling">
          <el-input-number :min="0.00" v-model="form.spAfterMilling" placeholder="请输入铣后规格" />
          <el-span>&nbsp;mm</el-span>
        </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="铣前重量" prop="spBeforeWeight">
              <el-input-number :min="0" v-model="form.spBeforeWeight" placeholder="请输入铣前重量" />
              <el-span>&nbsp;Kg</el-span>
            </el-form-item>
          </el-col>
          <el-col :span="8">
        <el-form-item label="铣后重量" prop="spAfterWeight">
          <el-input-number :min="0" v-model="form.spAfterWeight" placeholder="请输入铣后重量" />
          <el-span>&nbsp;Kg</el-span>
        </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-form-item label="质量判定" prop="qualityJudgment" >
          <el-select v-model="form.qualityJudgment" placeholder="请选择质量判定" @change="handleInput">
            ...
            <el-option
              v-for="dict in dict.type.sys_smx_zlpd"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
...
<script>
import { listSmx, getSmx, delSmx, addSmx, updateSmx } from "@/api/mes/md/smx";
...
import {addKp} from "@/api/mes/md/kp";
...
export default {
  ...
  dicts: ['sys_rz_ph', 'sys_smx_zlpd', 'sys_cys_gx'],
  data() {
    return {
      // 筛选条件，假设是根据工单状态进行筛选
      filterCondition: {
        gdType: '3',  // 比如筛选状态为 '3-双面铣' 的工单
      },
      //自动生成编码
      autoGenFlag:false,
      optType: undefined,
      ...
      queryParams: {
        pageNum: 1,
        pageSize: 10,
        dsmCode: null,  spBeforeWeight: null,   spAfterWeight: null,    workorderId: null,        workorderCode: null,        brand: null,        batchNumber: null,        millingCount: null,        spBeforeMilling: null,        spAfterMilling: null,        millingTime: null,        userId: null,        userName: null,        qualityJudgment: null,        abnormalCause: null,      },
      // 表单参数
      form: {},
      // 表单校验
      rules: {
        millingCount: [
          { required: true, message: "铣面次数不能为空", trigger: "blur" }
        ],        spBeforeMilling: [
          { required: true, message: "铣前规格不能为空", trigger: "blur" }
        ],        spAfterMilling: [
          { required: true, message: "铣后规格不能为空", trigger: "blur" }
        ],        weight: [
  ...
  methods: {
    ...
    pdScgd(event){
      ...
      listScgd(queryRz).then(response => {
        ...
        if(response.rows.length === 0){
          ...
          if(scgd.thisProcess !== '3'  && scgd.workStatus !== '3'){
            ...
            listSmx(queryRz).then(response => {
              ...
              this.form.millingCount = smxlistLength + 1;
              if(smxlistLength > 0){
                const smx = response.rows[0];
                this.form.spBeforeWeight = smx.spAfterWeight;
                this.form.spBeforeMilling = smx.spAfterMilling;
              }else{
                listRezha(queryRz).then(response => {
                  if(response.rows.length > 0){
                    this.form.spBeforeWeight = response.rows[0].weightAfterRoll;
                  }
                });
              }
            });
    ...
    async cysfun() {
      ...
      try {
        ...
        // const response = await addKp(customObjects);
    ...
    getList() {
      this.loading = true;
      listSmx(this.queryParams).then(response => {
        this.smxList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getSmx(doublesidedId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.doublesidedId != null) {
            updateSmx(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addSmx(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除铣面编号为"' + doublesidedIds + '"的数据项？').then(function() {
        return delSmx(doublesidedIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('md/smx/export', {
        ...this.queryParams
      }, `smx_${new Date().getTime()}.xlsx`)
    },
    //自动生成编码
    handleAutoGenChange(autoGenFlag){
      if(autoGenFlag){
        genCode('CAS_CODE').then(response =>{
          this.form.dsmCode = response;
        });
      }else{
        this.form.dsmCode = null;
      }
    }
  }
};
